function [Cu1,Cu2,Cu3,Cu4]=plots_case1(x1,x2,x3,x4,A,B,C,D,energy_case)
if (energy_case ==1)
    i=7.7; o=1;
end
    if (energy_case ==2)
        i=11.5; o=2;
    end
        if (energy_case ==3)
            i=19.6; o=3;
        end
            if (energy_case ==4)
                i=27; o=4;
            end
                if (energy_case ==5)
                    i=39.6; o=5;
                end
                    if (energy_case ==6)
                        i=62.4; o=6;
                    end
                        if (energy_case ==7)
                            i=200; o=7;
                        end

            
            p=0; 
for M=[x1(1,17) x1(1,16) x1(1,15) x1(1,14) x1(1,13) x1(1,12) x1(1,11) x1(1,10) x1(1,9)]
                 p=p+1;
       C1=0; C2=0; C3=0; C4=0; z=0;
                  for N=1:M
  chem=1477/(1+0.343*i);    
                
    z=z+exp(N*(chem/x1(1,7)*(1-sin(100*pi*(chem-x1(1,8)))/(100*pi*(chem-x1(1,8))))-x1(1,2)*N/M-x1(1,3)*(N/M)^2-x1(1,4)*(N/M)^3-x1(1,5)*(N/M)^4-x1(1,6)*(N/M)^5));
            
       
    C1=C1+N*exp(N*(chem/x1(1,7)*(1-sin(100*pi*(chem-x1(1,8)))/(100*pi*(chem-x1(1,8))))-x1(1,2)*N/M-x1(1,3)*(N/M)^2-x1(1,4)*(N/M)^3-x1(1,5)*(N/M)^4-x1(1,6)*(N/M)^5));
    
    
    
                  end
                  
                   m1=C1/z;
                   
                   Cu1=m1;
                   
                   
                  
                   hold on
                   figure(1)
                   plot(M,Cu1,'ok',M,A(o,p),'xr')
                   grid on
                   ylabel('$C_1$','Interpreter','latex')
                   xlabel('Number of wounded Nucleons (M)','Interpreter','latex')
                   title('Comparison $C_1^{exp}$ with $C_1^{th}$','Interpreter','latex')
                   hold off
                   
                   
                 
end

            
            p=0; 
for M=[x2(1,17) x2(1,16) x2(1,15) x2(1,14) x2(1,13) x2(1,12) x2(1,11) x2(1,10) x2(1,9)]
                 p=p+1;
       C1=0; C2=0; C3=0; C4=0; z=0;
                  for N=1:M
  chem=1477/(1+0.343*i);    
                
    z=z+exp(N*(chem/x2(1,7)*(1-sin(100*pi*(chem-x2(1,8)))/(100*pi*(chem-x2(1,8))))-x2(1,2)*N/M-x2(1,3)*(N/M)^2-x2(1,4)*(N/M)^3-x2(1,5)*(N/M)^4-x2(1,6)*(N/M)^5));
            
    C1=C1+N*exp(N*(chem/x2(1,7)*(1-sin(100*pi*(chem-x2(1,8)))/(100*pi*(chem-x2(1,8))))-x2(1,2)*N/M-x2(1,3)*(N/M)^2-x2(1,4)*(N/M)^3-x2(1,5)*(N/M)^4-x2(1,6)*(N/M)^5));
    
 
    C2=C2+N^2*exp(N*(chem/x2(1,7)*(1-sin(100*pi*(chem-x2(1,8)))/(100*pi*(chem-x2(1,8))))-x2(1,2)*N/M-x2(1,3)*(N/M)^2-x2(1,4)*(N/M)^3-x2(1,5)*(N/M)^4-x2(1,6)*(N/M)^5));
    
    
                  end
                   
                   m1=C1/z;
                   m2=C2/z;
                   
                   
                   Cu2=m2-m1^2;
                   
                   hold on
                   figure(2)
                   plot(M,Cu2,'ok',M,B(o,p),'xr')
                  grid on
                   ylabel('$C_2$','Interpreter','latex')
                   xlabel('Number of wounded Nucleons (M)','Interpreter','latex')
                   title('Comparison $C_2^{exp}$ with $C_2^{th}$','Interpreter','latex')
                   hold off
                   
               
                   
                   
                 
end

            
            p=0; 
for M=[x3(1,17) x3(1,16) x3(1,15) x3(1,14) x3(1,13) x3(1,12) x3(1,11) x3(1,10) x3(1,9)]
                 p=p+1;
       C1=0; C2=0; C3=0; C4=0; z=0;
                  for N=1:M
  chem=1477/(1+0.343*i);    
                
    z=z+exp(N*(chem/x3(1,7)*(1-sin(100*pi*(chem-x3(1,8)))/(100*pi*(chem-x3(1,8))))-x3(1,2)*N/M-x3(1,3)*(N/M)^2-x3(1,4)*(N/M)^3-x3(1,5)*(N/M)^4-x3(1,6)*(N/M)^5));
     C1=C1+N*exp(N*(chem/x3(1,7)*(1-sin(100*pi*(chem-x3(1,8)))/(100*pi*(chem-x3(1,8))))-x3(1,2)*N/M-x3(1,3)*(N/M)^2-x3(1,4)*(N/M)^3-x3(1,5)*(N/M)^4-x3(1,6)*(N/M)^5));
    
 
    C2=C2+N^2*exp(N*(chem/x3(1,7)*(1-sin(100*pi*(chem-x3(1,8)))/(100*pi*(chem-x3(1,8))))-x3(1,2)*N/M-x3(1,3)*(N/M)^2-x3(1,4)*(N/M)^3-x3(1,5)*(N/M)^4-x3(1,6)*(N/M)^5));
    
       
   
    C3=C3+N^3*exp(N*(chem/x3(1,7)*(1-sin(100*pi*(chem-x3(1,8)))/(100*pi*(chem-x3(1,8))))-x3(1,2)*N/M-x3(1,3)*(N/M)^2-x3(1,4)*(N/M)^3-x3(1,5)*(N/M)^4-x3(1,6)*(N/M)^5));
    
 
    
                  end
                  
                   m1=C1/z;
                   m2=C2/z;
                   m3=C3/z;
                 
                   Cu3=m3-3*m2*m1+2*m1^3;
                 
                 
                   hold on
                   figure(3)
                   plot(M,Cu3,'ok',M,C(o,p),'xr')
                   grid on
                   ylabel('$C_3$','Interpreter','latex')
                   xlabel('Number of wounded Nucleons (M)','Interpreter','latex')
                   title('Comparison $C_3^{exp}$ with $C_3^{th}$','Interpreter','latex')
                   hold off
 
                   
                 
end

            
            p=0; 
for M=[x4(1,17) x4(1,16) x4(1,15) x4(1,14) x4(1,13) x4(1,12) x4(1,11) x4(1,10) x4(1,9)]
                 p=p+1;
       C1=0; C2=0; C3=0; C4=0; z=0;
                  for N=1:M
  chem=1477/(1+0.343*i);    
                
    z=z+exp(N*(chem/x4(1,7)*(1-sin(100*pi*(chem-x4(1,8)))/(100*pi*(chem-x4(1,8))))-x4(1,2)*N/M-x4(1,3)*(N/M)^2-x4(1,4)*(N/M)^3-x4(1,5)*(N/M)^4-x4(1,6)*(N/M)^5));
           
            
    C1=C1+N*exp(N*(chem/x4(1,7)*(1-sin(100*pi*(chem-x4(1,8)))/(100*pi*(chem-x4(1,8))))-x4(1,2)*N/M-x4(1,3)*(N/M)^2-x4(1,4)*(N/M)^3-x4(1,5)*(N/M)^4-x4(1,6)*(N/M)^5));
    
 
    C2=C2+N^2*exp(N*(chem/x4(1,7)*(1-sin(100*pi*(chem-x4(1,8)))/(100*pi*(chem-x4(1,8))))-x4(1,2)*N/M-x4(1,3)*(N/M)^2-x4(1,4)*(N/M)^3-x4(1,5)*(N/M)^4-x4(1,6)*(N/M)^5));
    
       
   
    C3=C3+N^3*exp(N*(chem/x4(1,7)*(1-sin(100*pi*(chem-x4(1,8)))/(100*pi*(chem-x4(1,8))))-x4(1,2)*N/M-x4(1,3)*(N/M)^2-x4(1,4)*(N/M)^3-x4(1,5)*(N/M)^4-x4(1,6)*(N/M)^5));
    
        
    C4=C4+N^4*exp(N*(chem/x4(1,7)*(1-sin(100*pi*(chem-x4(1,8)))/(100*pi*(chem-x4(1,8))))-x4(1,2)*N/M-x4(1,3)*(N/M)^2-x4(1,4)*(N/M)^3-x4(1,5)*(N/M)^4-x4(1,6)*(N/M)^5));
    
    
                  end
                  
                   m1=C1/z;
                   m2=C2/z;
                   m3=C3/z;
                   m4=C4/z;
             
                   Cu4=m4-4*m3*m1-3*m2^2+12*m2*m1^2-6*m1^4;
              
                   

                   
                   
                   
                   
                   
                   hold on;
                   figure(4)
                   plot(M,Cu4,'ok',M,D(o,p),'xr')
                   grid on
                   ylabel('$C_4$','Interpreter','latex')
                   xlabel('Number of wounded Nucleons (M)','Interpreter','latex')
                   title('Comparison $C_4^{exp}$ with $C_4^{th}$','Interpreter','latex')
                   hold off
                  
                   
                 
end

end

    